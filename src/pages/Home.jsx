    ...
    if (this.state.hasError) {
      return (
        <div className="p-4 text-center">
          ...
          <button onClick={() => this.setState({ hasError: false })} className="px-4 py-2 bg-blue-500 text-white rounded">
            Try Again
          </button>
        </div>
      );
...
export default function Home() {
  ...
  const unseenOnboardingCards = ONBOARDING_CARDS.filter((c) => !effectiveSeenOnboardingIds.includes(c.id));
  const {
    subscriptionData,
    isOnTrial,
    isTrialExpired,
  ...
  }, [authLoading, tiktokAuthLoading, isLoggedIn, isTikTokLoggedIn, navigate]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      (!selectedAdAccountIds || selectedAdAccountIds.length === 0)
    ) {
  ...
  useEffect(() => {
    if (!subscriptionLoading && isTrialExpired && !userHasActiveAccess && !hasDismissedTrialPopup) {
  ...
  useEffect(() => {
    ...
    const initialTemplateName =
      preferredName && keys.includes(preferredName)
        ? preferredName
  ...
  }, [selectedAdAccount, adAccountSettings.copyTemplates, adAccountSettings.defaultTemplateName]);
  ...
  useEffect(() => {
    ...
    setImportedFiles((prev) => {
      ...
      const existingIds = new Set(prev.map((f) => (f.type === "image" ? f.hash : f.id)));
      const additions = payload.filter((f) => !existingIds.has(f.type === "image" ? f.hash : f.id));
      return [...prev, ...additions];
    });
  ...
  const handleWizardFinish = async ({ seenIds, fullyCompleted }) => {
    ...
    const idsToPersist = fullyCompleted && !hasSeenOnboarding ? ONBOARDING_CARDS.map((c) => c.id) : seenIds;
  ...
  const isFormFieldModified = useCallback(
    (fieldKeys) => {
  ...
  const handleAddVariant = useCallback(() => {
    const usedLetters = new Set(
      variants.filter((variant) => variant.id !== "default").map((variant) => variant.name.replace(/^(Form|Variant)\s+/, "")),
    );
    let letterIndex = 0;
    let nextLetter = "";
    while (!nextLetter) {
    ...
    setVariants((prev) => [
      ...prev.map((variant) => (variant.id === activeVariantId ? { ...variant, snapshot: currentSnapshot } : variant)),
  ...
  const handleDeleteVariant = useCallback(
    (variantId) => {
      ...
      if (wasActive) {
        ...
        setVariants((prev) => prev.map((variant) => (variant.id === "default" ? { ...variant, snapshot: null } : variant)));
      }
  ...
  const handleDeleteAllVariants = useCallback(() => {
    ...
    const defaultVariant = variants.find((variant) => variant.id === "default");
    const defaultSnapshot = activeVariantId === "default" ? captureCurrentSnapshot() : defaultVariant?.snapshot;
  ...
  }, [activeVariantId, captureCurrentSnapshot, fileVariantMap, groupVariantMap, postVariantMap, hydrateFromSnapshot, variants]);
  ...
  useEffect(() => {
    ...
    const groupedFileIds = new Set(fileGroups.flatMap((group) => group.fileIds || []));
  ...
  useEffect(() => {
    ...
    const fetchCampaignsForSelectedAccount = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/auth/fetch-campaigns?adAccountId=${selectedAdAccount}`, { credentials: "include" });
        const data = await res.json();
  ...
  const handleAdAccountChange = useCallback(async (value) => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/auth/fetch-campaigns?adAccountId=${adAccountId}`, { credentials: "include" });
      const data = await res.json();
      if (data.campaigns) {
        const priority = {
          ACTIVE: 1,
  ...
  const refreshAdSets = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/auth/fetch-adsets?campaignId=${selectedCampaign}`, { credentials: "include" });
      const data = await res.json();
      if (data.adSets) {
  ...
  const handleLocalAdSetCreated = useCallback(
    ({ newAdSetId, sourceAdSetId, name, campaignId }) => {
      if (!newAdSetId) {
        return;
      }
    ...
    <>
        ...
        <div className="mobile-message fixed inset-0 bg-white flex-col items-center justify-center p-6 z-[100] hidden">
          <div className="text-center max-w-md">
            ...
            <p className="text-gray-600 mb-6">
              Thanks for signing up! <br></br>Blip works best on a bigger screen. <br></br> We've sent you an email to help you<br></br> pick up from
              here.
            </p>
            <button
      ...
      <div className="w-full max-w-[1600px] mx-auto py-8 px-2 sm:px-4 md:px-6">
        <Header
          ...
          metaNotLinked={!isLoggedIn}
        />
        <div className="flex flex-col lg:flex-row gap-6 min-w-0">
          <div
            className={`flex-1 lg:flex-[55] min-w-0 space-y-6 ${!userHasActiveAccess || !isLoggedIn ? "pointer-events-none opacity-40 cursor-not-allowed select-none" : ""}`}
          >
            <AdAccountSettings
              isLoading={isLoading}
          ...
          <div
            className={`flex-1 lg:flex-[45] min-w-0 ${!userHasActiveAccess || !isLoggedIn ? "pointer-events-none opacity-40 cursor-not-allowed select-none" : ""}`}
          >
            <ErrorBoundary>
              <MediaPreview
      ...
      <AdAccountSelectionPopup isOpen={showAdAccountPopup} onClose={() => setShowAdAccountPopup(false)} />
