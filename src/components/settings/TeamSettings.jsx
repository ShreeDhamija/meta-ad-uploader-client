            ...
            <Card className="rounded-3xl shadow-lg shadow-gray-200/50">
                ...
                <CardContent>
                    ...
                    {teamMode === 'owner' && teamData && (
                        <div className="space-y-3">
                            <div className="bg-gray-50 p-3 rounded-xl cursor-pointer hover:bg-gray-100"
                                onClick={() => { navigator.clipboard.writeText(teamData.inviteCode); toast.success("Copied to clipboard!") }}>
                            ...
                            <p className="text-sm text-gray-600">Team: {teamData.teamName || teamName}</p>
                            <p className="text-sm text-gray-600">Total members: {teamData.members ? teamData.members.length + 1 : 1}</p>
                                    ...
                                    {teamData.members.map((member) => (
                                        <div key={member.id} className="flex items-center justify-between p-2 rounded-xl bg-gray-50">
                                            <div className="flex items-center gap-3">
                                                <img src={member.picture || '/default-avatar.png'} alt={member.name} className="w-6 h-6 rounded-full" />
                                                <span className="text-sm">{member.name}</span>
                                            </div>
                                            <Button onClick={() => handleRemoveMember(member.id)} variant="ghost" size="icon"
                                                className="h-8 w-8" disabled={deletingMemberId === member.id}>
