...
import { Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from "@/components/ui/select"
import { toast } from "sonner"
import { CirclePlus, Trash2, Download, Loader } from 'lucide-react'
import TextareaAutosize from 'react-textarea-autosize'
import { RotateLoader } from "react-spinners"
import TemplateIcon from '@/assets/icons/template.svg?react';
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
...
export default function CopyTemplates({
  ...
  useEffect(() => {
    ...
    const reducerIsEmpty = Object.keys(state.templates).length === 0;

    if (accountChanged || (hasParentData && reducerIsEmpty)) {
      dispatch({
        type: "SET_ALL",
        payload: {
          templates: parentTemplates || {},
          defaultName: parentDefaultName || "",
        },
      });
      ...
      lastAccountIdRef.current = selectedAdAccount;
    }
  }, [selectedAdAccount, parentTemplates, parentDefaultName, state.templates]);
    ...
    <div className="p-4 bg-[#f5f5f5] rounded-xl space-y-3 w-full max-w-3xl">
      <div className="flex items-start justify-between mb-6">
        <div className="flex flex-col gap-[12px]">
          ...
          <p className="text-xs text-gray-500 leading-tight">
            Add up to 5 Primary Texts and Headlines below, <br />
            Then save as a template. All changes are saved via the main button below.
          </p>
        </div>
        <div className="flex items-center">
          <Button
