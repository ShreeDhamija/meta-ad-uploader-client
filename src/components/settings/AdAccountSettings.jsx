...
export default function AdAccountSettings({ preselectedAdAccount, onTriggerAdAccountPopup, subscriptionData }) {
  ...
  const hasChanges = useMemo(() => {
    ...
    return (
      ...
      !isDeepEqual(copyTemplates, initialSettings.copyTemplates) ||
    ...
    <div className="space-y-6 w-full max-w-3xl">
      ...
      <fieldset disabled={!selectedAdAccount || loading}>
        <div className={!selectedAdAccount || loading ? "opacity-70 cursor-not-allowed space-y-6" : "space-y-6"}>
          ...
          <CopyTemplates
            selectedAdAccount={selectedAdAccount}
            // --- PASS STATE, NOT SETTINGS OBJECT ---
            parentTemplates={copyTemplates}
            parentDefaultName={defaultTemplateName}
            onSyncTemplates={setCopyTemplates}
            onSyncDefaultName={setDefaultTemplateName}
            onTemplateUpdate={handleTemplateUpdate}
          />
