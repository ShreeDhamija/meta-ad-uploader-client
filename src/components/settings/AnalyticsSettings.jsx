...
export default function AnalyticsSettings() {
    ...
    const fetchQuickStats = async () => {
        ...
        try {
            const response = await fetch(
                `${API_BASE_URL}/api/analytics/quick-stats?adAccountId=${selectedAdAccount}`,
                { credentials: 'include' }
            )
            const data = await response.json()
            if (response.ok) {
                ...
                <Popover open={openAdAccount} onOpenChange={setOpenAdAccount}>
                    <PopoverTrigger asChild>
                        <Button
                            variant="outline"
                            role="combobox"
                            className="w-[300px] justify-between rounded-2xl h-11 bg-white shadow-sm hover:bg-white"
                        >
                            {selectedAdAccountName}
                            <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                        </Button>
                        ...
                        <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-xl font-bold mt-1">{formatCurrency(quickStats.today.spend)}</p>
                        ...
                        <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-xl font-bold mt-1">{formatCurrency(quickStats.last7Days.spend)}</p>
                                    <p className="text-xs text-gray-400 mt-1">
                        ...
                        <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    {quickStats.trends.cprVsYesterday !== null && (
                                        <p className={cn("text-xs mt-1", formatPercent(quickStats.trends.cprVsYesterday, true)?.color)}>
                                            {formatPercent(quickStats.trends.cprVsYesterday, true)?.text} vs yesterday
                                        </p>
                                    )}
                                </div>
                        ...
                        <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-xl font-bold mt-1">{quickStats.today.results.toLocaleString()}</p>
                                    <p className="text-xs text-gray-400 mt-1 truncate max-w-[120px]">
                                        {quickStats.optimizationGoal?.replace('offsite_conversion.fb_pixel_', '').replace(/_/g, ' ') || 'conversions'}
                                    </p>
                                </div>
                                <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center">
                                            ...
                                            {adsetInsights.slice(0, 5).map((insight, idx) => (
                                                <div key={insight.adsetId} className="flex items-center justify-between bg-gray-50 rounded-xl p-3">
                                                    ...
                                                    <div className="text-right ml-4">
                                                        ...
                                                        {insight.today.cpr && (
                                                            <p className="text-xs text-gray-400">CPR: {formatCurrency(insight.today.cpr)}</p>
                                                        )}
                                                    </div>
                                                </div>
                                            ))}
                ...
                <CardContent className="p-4">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="text-sm text-gray-500">
                            <p className="font-medium text-gray-600 mb-1">How Analytics Works</p>
                            <ul className="space-y-1 text-xs">
                                <li>• <strong>Anomaly Detection:</strong> Alerts when cost per result spikes 50%+ vs 7-day average, or when spend exceeds 150% of daily budget (ABO only)</li>
                                ...
                                <li>• CBO campaigns are analyzed at campaign level, ABO campaigns at adset level</li>
                            </ul>
                        </div>
                    </div>
                </CardContent>
