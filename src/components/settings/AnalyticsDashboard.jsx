...
//     Target, BarChart3, Settings2, Activity, Zap, Eye, XCircle, Slack
// } from "lucide-react"
...
//         try {
//             const res = await fetch(
//                 `${API_BASE_URL}/api/analytics/weekly-insights?adAccountId=${selectedAdAccount}`,
//                 { credentials: 'include' }
//             )
//             const data = await res.json()
...
//     // Fetch charts whenever account changes
//     useEffect(() => {
//         if (selectedAdAccount) {
//             fetchDailyInsights()
//             fetchWeeklyInsights()
//         }
//     }, [selectedAdAccount, fetchDailyInsights, fetchWeeklyInsights])
...
//             try {
//                 const res = await fetch(`${API_BASE_URL}/api/slack/status`, { credentials: 'include' });
//                 const data = await res.json();
...
//         // Invalidate anomalies cache so next fetch uses new thresholds
...
export default function AnalyticsDashboard() {
    ...
    // ── Target KPI ──────────────────────────────────────────
    const [targetKPI, setTargetKPI] = useState("") // string for input binding
    const [targetKPISaved, setTargetKPISaved] = useState(false)
    ...
    // ── Target KPI: load from ad account settings ───────────
    // (If you're using useAdAccountSettings, wire this up there instead.
    //  This is a standalone approach that works either way.)
    useEffect(() => {
        if (!selectedAdAccount) return
        const loadTargetKPI = async () => {
            try {
                const res = await fetch(
                    `${API_BASE_URL}/settings/ad-account?adAccountId=${selectedAdAccount}`,
                    { credentials: 'include' }
                )
                const data = await res.json()
                const settings = data?.settings || data || {}
                if (settings.targetCPA && metricMode === 'cpr') {
                    setTargetKPI(String(settings.targetCPA))
                } else if (settings.targetROAS && metricMode === 'roas') {
                    setTargetKPI(String(settings.targetROAS))
                } else {
                    setTargetKPI("")
                }
            } catch (err) {
                console.error('Failed to load target KPI:', err)
            }
        }
        loadTargetKPI()
    }, [selectedAdAccount, metricMode])
    ...
    const handleModeChange = (mode) => {
        setMetricMode(mode)
        setModeAutoDetected(false)
        setTargetKPI("") // Clear target when switching modes
        // Clear mode-dependent data
        setRecommendations(null)
        setPoorAds(null)
        const prefix = `recs-${selectedAdAccount}`
    ...
    const handleTargetKPISave = async () => {
        if (!selectedAdAccount) return
        const value = parseFloat(targetKPI)
        if (targetKPI && (isNaN(value) || value <= 0)) {
            toast.error('Please enter a valid target number')
            return
        }

        try {
            const settingsPayload = metricMode === 'roas'
                ? { targetROAS: value || null, targetCPA: null }
                : { targetCPA: value || null, targetROAS: null }
            ...
            await fetch(`${API_BASE_URL}/settings/save`, {
                ...
                body: JSON.stringify({
                    adAccountId: selectedAdAccount,
                    adAccountSettings: settingsPayload,
                }),
            })
            setTargetKPISaved(true)
            setTimeout(() => setTargetKPISaved(false), 2000)
        } catch (err) {
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-1.5">
                        <div className="relative">
                            ...
                            {metricMode === 'cpr' && (
                                <span className="absolute left-7 top-1/2 -translate-y-1/2 text-xs text-gray-400">$</span>
                            )}
                            <input
                                type="number"
                                value={targetKPI}
                                onChange={(e) => { setTargetKPI(e.target.value); setTargetKPISaved(false) }}
                                onBlur={handleTargetKPISave}
                                onKeyDown={(e) => { if (e.key === 'Enter') { e.target.blur(); handleTargetKPISave() } }}
                                placeholder={metricMode === 'roas' ? 'Target ROAS' : 'Target CPA'}
                                className={cn(
                                    "w-[130px] h-11 text-sm border rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white",
                                    metricMode === 'cpr' ? "pl-9 pr-3" : "pl-7 pr-3"
                                )}
                            />
                            {metricMode === 'roas' && targetKPI && (
                                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400">x</span>
                            )}
                        </div>
                        {targetKPISaved && (
                            <span className="text-xs text-green-600 animate-in fade-in duration-300">Saved</span>
                        )}
                    </div>
                </div>
