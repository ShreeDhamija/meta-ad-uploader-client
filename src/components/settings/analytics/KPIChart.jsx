...
export default function KPIChart({ data, loading, mode, days, onDaysChange, targetKPI }) {
    const { chartData, campaigns, avgValue } = useMemo(() => {
        if (!data?.dailyInsights?.length) return { chartData: [], campaigns: [], avgValue: 0 }
    ...
    const targetValue = targetKPI ? parseFloat(targetKPI) : null
                ...
                {loading ? (
                    ...
                    <ResponsiveContainer width="100%" height={220}>
                        <LineChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                            ...
                            {/* Account average line (gray, dashed) */}
                            {avgValue > 0 && (
                                <ReferenceLine
                                    y={avgValue}
                                    stroke="#9ca3af"
