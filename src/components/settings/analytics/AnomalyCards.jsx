...
import { Badge } from "@/components/ui/badge"
import {
    AlertTriangle, CheckCircle2, Loader2, Eye, TrendingUp, Ban,
} from "lucide-react"
import { cn } from "@/lib/utils"

// ── Anomaly card for a single item ──────────────────────────────────────────

function AnomalyCard({ anomaly }) {
    const isCritical = anomaly.severity === 'critical'

    const typeConfig = {
        cpa_spike: { label: 'CPA Spike', icon: TrendingUp },
        overspend: { label: 'Overspend', icon: AlertTriangle },
        no_results: { label: 'No Results', icon: Ban },
        spend_velocity: { label: 'Spend Velocity', icon: TrendingUp },
    }
    const config = typeConfig[anomaly.type] || typeConfig.cpa_spike
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <p className="font-medium text-gray-900">
                                    {anomaly.adsetName || anomaly.campaignName}
                                </p>
                                <Badge variant="outline" className="text-[10px] px-1.5 py-0 rounded-full bg-gray-100 text-gray-500 border-gray-200">
                                    {config.label}
                                </Badge>
                            ...
                            <p className="text-sm text-gray-600 mt-0.5">{anomaly.message}</p>
                            {/* Show campaign name below for adset-level anomalies */}
                            {anomaly.adsetName && anomaly.campaignName && (
                                <p className="text-xs text-gray-400 mt-1">Campaign: {anomaly.campaignName}</p>
                            )}
                            ...
                            <div className="flex items-center gap-4 mt-2">
                                ...
                                {anomaly.type === 'no_results' && anomaly.details && (
                                    <>
                                        <span className="text-xs text-gray-500">
                                            Spent today: <span className="font-medium text-orange-600">${anomaly.details.todaySpend}</span>
                                        </span>
                                        <span className="text-xs text-gray-500">
                                            7d avg CPA: <span className="font-medium">${anomaly.details.weekCPA}</span>
                                        </span>
                                        <span className="text-xs font-medium text-red-600">
                                            0 conversions
                                        </span>
                                ...
                                {anomaly.type === 'spend_velocity' && anomaly.details && (
                                    <>
                                        <span className="text-xs text-gray-500">
                                            Today so far: <span className="font-medium text-orange-600">${parseFloat(anomaly.details.todaySpend).toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                                        </span>
                                        <span className="text-xs text-gray-500">
                                            Yesterday (same window): <span className="font-medium">${parseFloat(anomaly.details.yesterdayPaced).toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                                        </span>
                                        <span className="text-xs font-medium text-red-600">
                                            +{anomaly.details.pctIncrease}%
                                        </span>
                                        <span className="text-xs text-gray-400">
                                            {anomaly.details.elapsedPct}% of day elapsed
                                        </span>
                                    </>
                                )}
                            </div>
...
export default function AnomalyCards({ data, loading, thresholds }) {
    ...
    const anomalies = data?.anomalies || []
    const velocityAnomalies = data?.velocityAnomalies || []
    const nearThreshold = data?.nearThreshold || []
    const hasAnything = anomalies.length > 0 || velocityAnomalies.length > 0 || nearThreshold.length > 0
    ...
    return (
        <div className="space-y-5">
            ...
            {/* Campaign spend velocity section */}
            {velocityAnomalies.length > 0 && (
                <div className="space-y-2">
                    <p className="text-xs font-medium text-gray-500 px-1 flex items-center gap-1.5">
                        <TrendingUp className="w-3.5 h-3.5" />
                        Campaign Spend Velocity
                        <span className="text-gray-400 font-normal">· pacing vs yesterday</span>
                    </p>
                    {velocityAnomalies.map((anomaly, i) => (
                        <AnomalyCard key={anomaly.id || i} anomaly={anomaly} />
                    ))}
                </div>
            )}

            {/* Adset-level anomalies */}
            {anomalies.length > 0 && (
                <div className="space-y-2">
                    {velocityAnomalies.length > 0 && (
                        <p className="text-xs font-medium text-gray-500 px-1 flex items-center gap-1.5 mt-2">
                            <AlertTriangle className="w-3.5 h-3.5" />
                            Adset Anomalies
                            <span className="text-gray-400 font-normal">· CPA, spend, and conversion issues</span>
                        </p>
                    )}
