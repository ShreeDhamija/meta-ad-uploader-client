...
function SortableMediaItem({ file, index, isCarouselAd, videoThumbs, onRemove }) {
  const { attributes, listeners, setActivatorNodeRef, setNodeRef, transform, transition, isDragging } = useSortable({
    id: file.isDrive ? file.id : file.name,
  })
    ...
    <div ref={setNodeRef} style={style} className={`relative group ${isDragging ? "opacity-50" : ""}`}>
      ...
      <div className="overflow-hidden rounded-xl shadow-lg border border-gray-200">
        {(file.type || file.mimeType || "").startsWith("video/") ? (
          ...
          <img
            src={
              file.isDrive ? `https://drive.google.com/thumbnail?id=${file.id}&sz=w400-h300` : URL.createObjectURL(file)
            }
            alt={file.name}
            className="w-full h-auto object-cover"
            onError={(e) => {
  ...
  return (
    <>
      {files.length > 0 ? (
        ...
        >
          <CardHeader className="flex flex-row justify-between items-start flex-nowrap w-full p-6">
            <div className="flex flex-col items-start min-w-0 flex-1">
              <CardTitle className="text-left">Uploads Preview</CardTitle>
              <CardDescription className="text-left">
                {`${files.length} file${files.length > 1 ? "s" : ""} selected`}
                {isCarouselAd && (
                  <span className="block text-xs text-gray-500 mt-1">Drag to change order of carousel cards</span>
                )}
              </CardDescription>
            </div>
            <Button
        ...
        <div className="sticky top-4 w-full mx-auto" style={{ height: "calc(100vh - 140px)" }}>
          ...
          >
            <div className="bg-white rounded-2xl shadow-md p-8 max-w-sm w-full mx-4 text-center min-h-[500px] border border-gray-100 flex flex-col justify-center">
              {/* UPLOAD Text */}
              <div className="mb-8">
                <img src={Uploadimg || "/placeholder.svg"} alt="Upload illustration" className="w-24 h-5 mx-auto" />
              </div>

              {/* Your server image */}
              <div className="mb-8">
                <img
                  src={RocketImg || "/placeholder.svg"}
                  alt="Upload illustration"
                  className="w-[204px] h-[260px] max-w-xs mx-auto"
                />
              </div>
