          ...
          <CardHeader className="flex flex-row justify-between items-start flex-nowrap w-full">
            <div className="flex flex-col items-start">
              ...
              <CardDescription className="text-left">
                {`${files.length + driveFiles.length + (dropboxFiles?.length || 0) + importedFiles.length} file${(files.length + driveFiles.length + (dropboxFiles?.length || 0) + importedFiles.length) > 1 ? "s" : ""} selected`}
                {isCarouselAd && !enablePlacementCustomization && (
