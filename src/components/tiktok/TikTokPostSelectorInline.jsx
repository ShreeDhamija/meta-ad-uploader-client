    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-shrink-0 space-y-3 pb-2">
        ...
        {filteredPosts.length > 0 && (
          <div className="grid grid-cols-[20px_48px_1fr] gap-3 px-3 py-2.5 text-xs font-semibold text-white bg-zinc-800 rounded-xl items-center shadow-sm">
            <div></div>
            <div>Cover</div>
            <div>Caption</div>
          </div>
          ...
          <div className="space-y-1.5 pb-4">
            {filteredPosts.map((post) => {
              ...
              return (
                <label
                  key={post.id}
                  className={`grid grid-cols-[auto_48px_1fr] gap-3 items-center p-3 rounded-2xl border cursor-pointer transition-all duration-150 ${isSelected
